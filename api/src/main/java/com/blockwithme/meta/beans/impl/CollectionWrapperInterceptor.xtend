...
/**
 * <code>CollectionWrapperInterceptor</code> is a WrapperInterceptor for CollectionBeans.
 *
 * @author monster
 */
class CollectionWrapperInterceptor<E> extends DefaultCollectionInterceptor<E> {

    /** Default instance */
    public static val INSTANCE = new CollectionWrapperInterceptor()
	...
	override getObjectAtIndex(_CollectionBean<E> instance, int index, E value) {
        val delegate = instance.getDelegate()
        if ((delegate === null) || instance.isSelected(index)) {
            return value
        }
        delegate.get(index)
	}
