...
/**
 * A Processor that only works on Class types.
 *
 * @author monster
 */
class ClassProcessor extends Processor<ClassDeclaration, MutableClassDeclaration> {
	...
	/** Returns true, if this type is an Class that should be processed. */
	override boolean accept(TypeDeclaration td) {
		(td instanceof ClassDeclaration) && super.accept(td)
	}
}